...
type TranslationState = { status: "pending" | "running" | "completed" | "needs_review" | "failed"; engine?: string; source_url?: string | null; error?: string | null; previewUrl?: string | null; markdownUrl?: string | null; candidateUrl?: string | null; pdfMonoUrl?: string | null; pdfDualUrl?: string | null; structureReview?: { ambiguous: string[]; reviewIssues: string[]; objects: Array<{ id: string; kind: string; caption: string | null; captionKind: string | null; captionNumber: number | null; asset: string | null }> }; source_chars?: number; translated_chars?: number; progress_phase?: string; progress_current?: number; progress_total?: number; progress_message?: string | null; started_at?: string | null; updated_at?: string };
...
function translationProgressPercent(translation: TranslationState) {
  ...
  return ({ queued: 5, starting: 8, downloading: 12, parsing: 22, extracting: 22, source_quality_check: 26, binding_review: 32, rendering: 88, validating: 96 } as Record<string, number>)[translation.progress_phase || "queued"] || 5;
}
      ...
      <article className="mx-auto max-w-4xl rounded-xl border border-gray-200 bg-white p-8 shadow-sm">
        ...
        {translation && <section className="mt-4 rounded-lg border border-indigo-100 bg-indigo-50/60 p-4 text-sm">
          <div className="flex flex-wrap items-center justify-between gap-2">
           ...
           {(translation.status === "completed" || translation.status === "needs_review") && translation.previewUrl && <div className="mt-2 flex flex-wrap gap-2"><a href={translation.previewUrl} className={`inline-block rounded-md px-3 py-1.5 text-xs font-medium text-white ${translation.status === "needs_review" ? "bg-amber-600 hover:bg-amber-700" : "bg-indigo-600 hover:bg-indigo-700"}`}>{translation.status === "needs_review" ? "打开待复核译文" : "打开中文译文预览"}</a>{translation.markdownUrl && <a href={translation.markdownUrl} className="inline-block rounded-md border border-indigo-200 bg-white px-3 py-1.5 text-xs font-medium text-indigo-700 hover:bg-indigo-50">下载 Markdown</a>}{translation.pdfMonoUrl && <a href={translation.pdfMonoUrl} className="inline-block rounded-md border border-indigo-200 bg-white px-3 py-1.5 text-xs font-medium text-indigo-700 hover:bg-indigo-50">下载单语 PDF</a>}{translation.pdfDualUrl && <a href={translation.pdfDualUrl} className="inline-block rounded-md border border-indigo-200 bg-white px-3 py-1.5 text-xs font-medium text-indigo-700 hover:bg-indigo-50">下载双语 PDF</a>}</div>}
           {translation.status === "needs_review" && <div className="mt-2"><p className="text-xs leading-5 text-amber-800">{translationCanAutoRepair(translation) ? "检测到可自动修复的源 OCR 异常，点击上方按钮重新解析并翻译。" : `译文已生成，但质量校验未通过：${translation.error || "请打开待复核译文，结合原文检查章节、公式和图表。"}`}</p>{translation.structureReview?.objects?.length ? <p className="mt-1 text-xs leading-5 text-amber-700">待判断对象：{translation.structureReview.objects.slice(0, 8).map((item) => item.id).join("、")}{translation.structureReview.objects.length > 8 ? " 等" : ""}</p> : null}{translationCanAutoRepair(translation) && translation.error && <p className="mt-1 text-xs leading-5 text-amber-700">诊断：{translation.error}</p>}{translation.candidateUrl && <a href={translation.candidateUrl} className="mt-2 inline-block rounded-md border border-amber-300 bg-white px-3 py-1.5 text-xs font-medium text-amber-800 hover:bg-amber-50">下载待复核译文</a>}</div>}
          ...
          {translationIsActive(translation) && <div className="mt-3">
            ...
            <div className="mt-2 h-2 overflow-hidden rounded-full bg-indigo-100" role="progressbar" aria-label="论文翻译进度" aria-valuemin={0} aria-valuemax={100} aria-valuenow={translationProgressPercent(translation)}><div className={`h-full rounded-full bg-indigo-500 transition-all duration-500 ${translation.progress_total ? "" : "animate-pulse"}`} style={{ width: `${translationProgressPercent(translation)}%` }} /></div>
             <div className="mt-2 flex flex-wrap justify-between gap-2 text-[11px] text-indigo-600"><span>{translation.engine === "pdf2zh-next" ? "下载 PDF → pdf2zh-next 版面翻译 → PDF 校验" : "下载 PDF → 结构解析 → 分块翻译 → 质量校验"}</span>{translation.updated_at && <span>更新于 {new Date(`${translation.updated_at.replace(" ", "T")}Z`).toLocaleTimeString("zh-CN", { hour12: false })}</span>}</div>
          </div>}
        </section>}
