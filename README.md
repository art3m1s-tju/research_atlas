...
4. 查看命令输出的 `tailscale serve status` 地址，在 iPhone Safari 打开。也可以使用主机的 Tailscale `100.x.y.z` 地址加同一个端口访问，例如 `http://100.x.y.z:43117`。
...
端口统一由 `.env.local` 中的 `ATLAS_PORT` 控制，默认是 `43117`，`npm run dev`、`npm run start`、一键启动和 Tailscale 启动都会使用同一个端口。如果改成其他端口，所有入口会一起使用新端口；iPhone 通过裸 IP 访问时也使用同一个端口。`tailscale serve status` 输出的 HTTPS 地址通常不需要手动填写端口。
